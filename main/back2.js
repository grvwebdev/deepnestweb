'use strict';
importScripts('util/geometryutil.js');
importScripts('util/clipper.js');
importScripts('util/d3-polygon.js');
...
// window.fq = new window.FileQueue(500);
window.nfpcache = {};
      ...
      if(pairs.length > 0){
          ...
          var spawncount = 0;
            ...
            p._spawnMapWorker = function (i, cb, done, env, wrk){
				...
				var psmsgData = {process:'background-progress',index: index, progress: 0.5*(spawncount++/pairs.length)}
				postMessage(psmsgData);
...
async function getOuterNfp(A, B, inside){
	...
	// console.log('computing minkowski: ',A, B);
	// console.time('addon');
...
async function placeParts(sheets, parts, config, nestindex){
	...
	while(parts.length > 0){
		...
		for(i=0; i<parts.length; i++){
			...
			var psmsgData = {process:'background-progress', index: nestindex, progress: 0.5 + 0.5*(placednum/totalnum)}
			postMessage(psmsgData);
	...
	var psmsgData = {process:'background-progress',index: nestindex, progress: -1}
	postMessage(psmsgData);
	// console.log({placements: allplacements, fitness: fitness, area: sheetarea, mergedLength: totalMerged });
